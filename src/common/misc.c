	...
	for (i = 0; i < src_nr; i++) {
		...
		if (alloc) {
			...
			if (!copy_ptr) {
				...
				for (j = 0; j < num; j++) {
					if (src_sgl[i].sg_iovs[j].iov_buf_len == 0)
						continue;

					rc = daos_iov_alloc(&dst_sgl[i].sg_iovs[j],
...
{
	return daos_sgls_copy_internal(dst, dst_nr, src, src_nr, false, true, false,
				       true);
}
